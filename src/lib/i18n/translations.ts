export const translations = {
  en: {
    // Navigation
    dashboard: 'Dashboard',
    dashboardSubtitle: 'BidNest Dashboard',
    members: 'Members',
    membersLower: 'members',
    of: 'of',
    overallCollectionProgress: 'Overall Collection Progress',
    monthProgress: 'Month Progress',
    allMonthsCombinedBalance: 'All Months - Combined Balance',
    combinedBalance: 'Combined Balance',
    clickToExpand: '(click a row to expand month breakdown)',
    update: 'Update',
    due: 'Due',
    paid: 'Paid',
    note: 'Note',
    groups: 'Chit Groups',
    group: 'Group',
    auctions: 'Auctions',
    payments: 'Payments',
    profile: 'Profile',
    logout: 'Logout',

    // Dashboard
    welcome: 'Welcome back',
    totalGroups: 'Total Groups',
    totalMembers: 'Total Members',
    totalCollected: 'Total Collected',
    activeGroups: 'Active Groups',

    // Members
    addMember: 'Add Member',
    editMember: 'Edit Member',
    memberName: 'Full Name',
    nickname: 'Nickname',
    mobile: 'Mobile Number',
    upiId: 'UPI ID',
    upiIds: 'UPI IDs',
    memberId: 'Member ID',
    status: 'Status',
    active: 'Active',
    inactive: 'Inactive',
    actions: 'Actions',
    search: 'Search by name, nickname or mobile...',
    noMembers: 'No members found',
    allStatuses: 'All Statuses',
    addFirstMember: 'Add your first member to get started.',
    deactivate: 'Deactivate',
    saveChanges: 'Save Changes',
    cancel: 'Cancel',

    // Groups
    newGroup: 'New Group',
    groupName: 'Group Name',
    editGroup: 'Edit Group',
    groupDetails: 'Group Details',
    totalAmount: 'Total Amount',

    duration: 'Duration (months)',
    monthlyAmount: 'Monthly Amount',
    commissionType: 'Commission Type',
    commissionValue: 'Commission',
    roundOff: 'Round Off Value',
    createGroup: 'Create Group',
    noGroups: 'No chit groups found',
    monthly: 'Monthly',
    commission: 'Commission',
    tickets: 'Tickets',
    progress: 'Progress',
    viewManage: 'View & Manage',
    allGroups: 'All Groups',
    allGroupsCombined: 'All Groups Combined',
    allGroupsParentheses: '(All Groups)',
    selectGroup: 'Select a group...',

    // Auctions
    conductAuction: 'Conduct Auction',
    totalAuctions: '{count} total auctions',
    monthNumber: 'Month Number',
    originalBid: 'Original Bid',
    winner: 'Winner',
    winnerPayout: 'Winner Payout',
    perMember: 'Per Member',
    toCollect: 'To Collect',
    carryNext: 'Carry Next',
    noAuctions: 'No auctions conducted yet.',
    noAuctionsDescription: 'Conduct your first auction to get started.',
    lastAuction: 'Last Auction',
    lastMonthBidFixed: 'Last month - bid fixed to commission cap.',
    editTicket: 'Edit Ticket',
    monthlyCollectionSummary: 'Monthly Collection Summary',

    // Payments
    recordPayment: 'Record Payment',
    amountPaid: 'Amount Paid',
    paymentMethod: 'Payment Method',
    paymentDate: 'Payment Date',
    date: 'Date',
    notes: 'Notes',
    notesPlaceholder: 'Optional notes about this payment',
    enterNotesPlaceholder: 'Enter notes...',
    saveNotes: 'Save Notes',
    saving: 'Saving...',
    clickToAddNotes: 'Click to add notes',
    cash: 'Cash',
    upi: 'UPI',
    bankTransfer: 'Bank Transfer',
    monthToPayFor: 'Month to Pay For',
    amountToPay: 'Amount to Pay',
    max: 'max',
    completed: 'Completed',
    partial: 'Partial',
    pending: 'Pending',
    noPayments: 'No payments found',
    noPaymentsDescription: 'Create a chit group to start tracking payments.',
    tracking: 'Payment Tracker',
    trackingSubtitle: 'Track monthly payment status for each member',
    memberPaymentStatus: 'Member Payment Status',

    // Tickets
    addTicket: 'Add Ticket',
    ticketNumber: 'Ticket Number',
    selectMember: 'Select member...',
    selectWinner: 'Select winner...',
    selectMonth: 'Select month...',
    allMonthsCombined: 'All Months (Combined)',
    monthLabel: 'Month',
    months: 'Months',
    ticketShort: 'Ticket #',

    // Statuses
    cancelled: 'Cancelled',

    // Profile
    editProfile: 'Edit Profile',
    changePassword: 'Change Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    accountDetails: 'Account Details',
    memberSince: 'Member Since',
    registeredMembers: 'registered members',

    // Member Detail
    totalTickets: 'Total Tickets',
    groupsWon: 'Groups Won',
    totalWon: 'Total Won',
    totalOwed: 'Total Owed',
    balanceDue: 'Balance Due',
    wonAuction: 'Winner',
    wonMonths: 'Won Month(s)',
    monthByMonth: 'Month-by-Month Breakdown',
    amountDue: 'Amount Due',
    balance: 'Balance',
    settled: 'Settled',
    notInAnyGroup: 'Not assigned to any chit group yet.',

    // Common
    overview: 'Overview',
    total: 'total',
    all: 'All',
    month: 'Month',
    groupNotFoundDescription: 'Group not found or you do not have access.',
    noTicketsAssigned: 'No tickets assigned yet.',
    unknown: 'Unknown',
    back: 'Back',
    loading: 'Loading...',
    notFound: 'Not Found',
    autoCalculated: 'Auto calculated',
    optional: 'optional',
    contactInfo: 'Contact Information',
    memberInfo: 'Member Info',
    joinedOn: 'Joined',
    created: 'Created',
    startDate: 'Start Date',
    paymentProgress: 'Payment progress',
    auctionsCompleted: 'Auctions completed',
    carryFromPrev: 'Prev.',
    calculationPreview: 'Calculation Preview',
    winnerGets: 'Winner Gets',
    baseMonthly: 'Base Monthly',
    dividend: 'Dividend',
    rawDividend: 'Raw Div.',
    rawPerMemberDiv: 'Raw/Mem',
    perMemberDiv: 'Per Member Div.',
    roundoffCarry: 'Roundoff Carry',
    roundoffDividend: 'Roundoff Dividend',
    eachMemberPays: 'Each Pays',
    noEligibleTickets: 'No eligible tickets (all have won)',
    fixedAmount: 'Fixed Amount',
    percentage: 'Percentage',

    // Payments Extended
    totalInflow: 'Total Inflow',
    totalPayouts: 'Total Payouts',
    commissionEarned: 'Commission Earned',
    netBalance: 'Net Balance',
    totalInflowDesc: 'Money collected from members',
    totalPayoutsDesc: 'Winning amounts disbursed',
    ofInflow: 'of inflow',
    surplus: 'Surplus',
    deficit: 'Deficit',
    groupBreakdown: 'Group Breakdown',
    recentPayments: 'Recent Payments',
    member: 'Member',
    totalPaid: 'Total Paid',
    totalDue: 'Total Due',
    remaining: 'Remaining',
    payingMonths: 'paying months',
    payment: 'Payment',
    allPayments: 'All Payments',
    paymentHistory: 'Payment History',
    more: 'more',

    // Profile Extended
    activeAccount: 'Active Account',
    personalInformation: 'Personal Information',
    updatePassword: 'Update Password',

    noGroupSelected: 'No group selected',

    // Toast / messages
    groupCreated: 'Chit group created!',
    groupUpdated: 'Group updated!',
    groupCancelled: 'Chit group cancelled',
    memberAdded: 'Member added!',
    memberUpdated: 'Member updated!',
    memberDeactivated: 'Member deactivated',
    ticketAdded: 'Ticket added!',
    paymentRecorded: 'Payment recorded!',
    passwordUpdated: 'Password updated',
    failedToLoad: 'Failed to load',

    // Reports
    downloadReport: 'Download Report',
    downloadPdf: 'Download PDF',
    selectLanguage: 'Select Language',
    english: 'English',
    tamil: 'Tamil',
    allGroupsCombinedReport: 'All Groups Combined',
    eachGroupSeparate: 'Each Group Separate',
    selectReportType: 'Select Report Type',
    generating: 'Generating...',
    groupReport: 'Group Report',
    auctionReport: 'Auction Report',
    memberReport: 'Member Report',
    chitPool: 'Chit Pool',
    bidDiscount: "Winner's Sacrifice",
    bidAmount: 'Bid Amount',
    result: 'Result',
    totalPool: 'Dividend Pool',
    rawPerMember: 'Raw/Mem',
    roundoffBalance: 'Rounding Balance',
    fixed: 'Fixed',
    dividendPool: 'Dividend Pool Calculation',
    monthlyCollection: 'Monthly Collection',
    carryFromPrevious: 'Prev.',
    carryToNext: 'Carry Fwd.',
    monthlyContribution: 'Monthly Contribution',
    dividendPerMember: 'Div./Mem',
    roundedDividend: 'Rnd. Div.',
  },

  ta: {
    // Navigation
    dashboard: 'டாஷ்போர்டு',
    dashboardSubtitle: 'பிட்நெஸ்ட் டாஷ்போர்டு',
    members: 'உறுப்பினர்கள்',
    membersLower: 'உறுப்பினர்கள்',
    of: 'இல்',
    overallCollectionProgress: 'ஒட்டுமொத்த தொகுப்பு முன்னேற்றம்',
    monthProgress: 'மாத முன்னேற்றம்',
    allMonthsCombinedBalance: 'அனைத்து மாதங்கள் — ஒருங்கிணைக்கப்பட்ட இருப்பு',
    combinedBalance: 'ஒருங்கிணைக்கப்பட்ட இருப்பு',
    clickToExpand: '(மாத விவரங்களை விரிவாக்க ஒரு வரியை கிளிக் செய்யவும்)',
    update: 'புதுப்பி',
    due: 'நிலுவை',
    paid: 'செலுத்தப்பட்டது',
    note: 'குறிப்பு',
    groups: 'சீட்டு குழுக்கள்',
    group: 'குழு',
    auctions: 'ஏலங்கள்',
    payments: 'கட்டணங்கள்',
    profile: 'சுயவிவரம்',
    logout: 'வெளியேறு',

    // Dashboard
    welcome: 'மீண்டும் வரவேற்கிறோம்',
    totalGroups: 'மொத்த குழுக்கள்',
    totalMembers: 'மொத்த உறுப்பினர்கள்',
    totalCollected: 'மொத்த வசூல்',
    activeGroups: 'செயலில் உள்ள குழுக்கள்',

    // Members
    addMember: 'சேர்',
    editMember: 'உறுப்பினரை திருத்து',
    memberName: 'முழு பெயர்',
    nickname: 'செல்லப் பெயர்',
    mobile: 'மொபைல் எண்',
    upiId: 'UPI ஐடி',
    upiIds: 'UPI ஐடி',
    memberId: 'உறுப்பினர் ID',
    status: 'நிலை',
    active: 'செயலில்',
    inactive: 'செயலற்றது',
    actions: 'செயல்கள்',
    search: 'பெயர் அல்லது மொபைல் மூலம் தேடு...',
    noMembers: 'உறுப்பினர்கள் இல்லை',
    allStatuses: 'அனைத்து நிலைகள்',
    addFirstMember: 'தொடங்க முதல் உறுப்பினரை சேர்க்கவும்.',
    deactivate: 'செயலிழக்கச் செய்',
    saveChanges: 'மாற்றங்களை சேமி',
    cancel: 'ரத்து செய்',

    // Groups
    newGroup: 'குழு',
    groupName: 'குழு பெயர்',
    editGroup: 'குழு திருத்து',
    groupDetails: 'குழு விவரம்',
    totalAmount: 'மொத்த தொகை',

    duration: 'கால அளவு (மாதங்கள்)',
    monthlyAmount: 'மாதாந்திர தொகை',
    commissionType: 'கமிஷன் வகை',
    commissionValue: 'கமிஷன்',
    roundOff: 'சுற்று மதிப்பு',
    createGroup: 'குழுவை உருவாக்கு',
    noGroups: 'சீட்டு குழுக்கள் இல்லை',
    monthly: 'மாதாந்திரம்',
    commission: 'கமிஷன்',
    tickets: 'டிக்கெட்டுகள்',
    progress: 'முன்னேற்றம்',
    viewManage: 'நிர்வகிக்கவும்',
    allGroups: 'அனைத்து சீட்டு குழுக்கள்',
    allGroupsCombined: 'அனைத்து சீட்டு குழுக்களும் ஒருங்கிணைக்கப்பட்டது',
    allGroupsParentheses: '(அனைத்து குழுக்கள்)',
    selectGroup: 'ஒரு குழுவை தேர்ந்தெடுக்கவும்...',

    // Auctions
    conductAuction: 'ஏலம் நடத்து',
    totalAuctions: '{count} மொத்த ஏலங்கள்',
    monthNumber: 'மாத எண்',
    originalBid: 'அசல் ஏலம்',
    winner: 'வெற்றியாளர்',
    winnerPayout: 'வெற்றியாளர் தொகை',
    perMember: 'ஒவ்வொரு உறுப்பினருக்கும்',
    toCollect: 'வசூலிக்க',
    carryNext: 'அடுத்த மாதம் இருப்பு',
    noAuctions: 'இன்னும் ஏலம் நடக்கவில்லை.',
    noAuctionsDescription: 'தொடக்கத்திற்கு உங்கள் முதல் ஏலத்தை நடத்துங்கள்.',
    lastAuction: 'கடைசி ஏலம்',
    lastMonthBidFixed: 'கடைசி மாதம் — ஏலம் கமிஷன் தொகைக்கு நிர்ணயிக்கப்பட்டது.',
    editTicket: 'டிக்கெட்டை திருத்து',

    // Payments
    recordPayment: 'கட்டணம் பதிவு செய்',
    amountPaid: 'செலுத்திய தொகை',
    paymentMethod: 'கட்டண முறை',
    paymentDate: 'கட்டண தேதி',
    date: 'தேதி',
    notes: 'குறிப்புகள்',
    notesPlaceholder: 'இந்த கட்டணத்தைப் பற்றிய விருப்ப குறிப்புகள்',
    enterNotesPlaceholder: 'குறிப்புகளை உள்ளீடு செய்...',
    saveNotes: 'குறிப்புகளை சேமி',
    saving: 'சேமிக்கப்படுகிறது...',
    clickToAddNotes: 'குறிப்புகளைச் சேர்க்க கிளிக் செய்யவும்',
    cash: 'ரொக்கம்',
    upi: 'UPI',
    bankTransfer: 'வங்கி பரிமாற்றம்',
    monthToPayFor: 'எந்த மாதத்திற்கு கட்டணம் செலுத்த?',
    amountToPay: 'செலுத்த வேண்டிய தொகை',
    max: 'அதிகபட்சம்',
    completed: 'முடிந்தது',
    partial: 'பகுதி',
    pending: 'நிலுவையில்',
    noPayments: 'கட்டணங்கள் இல்லை',
    noPaymentsDescription: 'கட்டணங்களை கண்காணிக்க ஒரு சீட்டு குழுவை உருவாக்கவும்.',
    tracking: 'கட்டண கண்காணிப்பு',
    trackingSubtitle: 'ஒவ்வொரு உறுப்பினரின் மாதாந்திர கட்டண நிலையை கண்காணிக்கவும்',
    memberPaymentStatus: 'உறுப்பினர் கட்டண நிலை',

    // Tickets
    addTicket: 'டிக்கெட் சேர்',
    ticketNumber: 'டிக்கெட் எண்',
    selectMember: 'உறுப்பினரை தேர்ந்தெடு...',
    selectWinner: 'வெற்றியாளரை தேர்ந்தெடு...',
    selectMonth: 'மாதம் தேர்ந்தெடு...',
    allMonthsCombined: '📊 அனைத்து மாதங்கள் (ஒருங்கிணைக்கப்பட்டது)',
    monthLabel: 'மாதம்',
    months: 'மாதங்கள்',
    ticketShort: 'டிக்கெட் #',

    // Statuses
    cancelled: 'ரத்து',

    // Profile
    editProfile: 'சுயவிவரம் திருத்து',
    changePassword: 'கடவுச்சொல் மாற்று',
    newPassword: 'புதிய கடவுச்சொல்',
    confirmPassword: 'கடவுச்சொல் உறுதிப்படுத்து',
    accountDetails: 'கணக்கு விவரங்கள்',
    memberSince: 'உறுப்பினர் ஆனது',
    registeredMembers: 'பதிவு செய்யப்பட்ட உறுப்பினர்கள்',

    // Member Detail
    totalTickets: 'மொத்த டிக்கெட்டுகள்',
    groupsWon: 'வெற்றி பெற்ற குழுக்கள்',
    totalWon: 'மொத்த வெற்றி தொகை',
    totalOwed: 'மொத்த செலுத்த வேண்டியது',
    balanceDue: 'நிலுவை தொகை',
    wonAuction: 'ஏலம் வென்றது',
    wonMonths: 'வென்ற மாதம்',
    monthByMonth: 'மாதாந்திர விவரம்',
    amountDue: 'வேண்டிய தொகை',
    balance: 'நிலுவை',
    settled: 'தீர்வு',
    notInAnyGroup: 'இன்னும் எந்த சீட்டு குழுவிலும் சேர்க்கப்படவில்லை.',

    // Common
    overview: 'மேலோட்டம்',
    total: 'மொத்தம்',
    all: 'அனைத்து',
    month: 'மாதம்',
    groupNotFoundDescription: 'குழு கிடைக்கவில்லை அல்லது உங்களுக்கு அணுகல் இல்லை.',
    noTicketsAssigned: 'இன்னும் டிக்கெட்டுகள் ஒதுக்கப்படவில்லை.',
    unknown: 'அறியப்படாதது',
    back: 'பின்னால்',
    loading: 'ஏற்றுகிறது...',
    notFound: 'கிடைக்கவில்லை',
    autoCalculated: 'தானாக கணக்கிடப்படும்',
    optional: 'விரும்பினால்',
    contactInfo: 'தொடர்பு தகவல்',
    memberInfo: 'உறுப்பினர் தகவல்',
    joinedOn: 'சேர்ந்தது',
    created: 'உருவாக்கப்பட்டது',
    startDate: 'தொடக்க தேதி',
    paymentProgress: 'கட்டண முன்னேற்றம்',
    auctionsCompleted: 'முடிந்த ஏலங்கள்',
    carryFromPrev: 'இருப்பு',
    calculationPreview: 'கணக்கீட்டு முன்னோட்டம்',
    winnerGets: 'வெற்றியாளர் பெறும் தொகை',
    baseMonthly: 'அடிப்படை மாதாந்திரம்',
    dividend: 'ஈவுத்தொகை',
    rawDividend: 'கச்சா ஈவுத்தொகை',
    rawPerMemberDiv: 'மொத்தம்/ஒவ்வொருவருக்கு',
    perMemberDiv: 'ஒவ்வொருவருக்கான ஈவுத்தொகை',
    roundoffCarry: 'சுற்று மதிப்பு கடன்',
    roundoffDividend: 'சுற்று மதிப்பு ஈவுத்தொகை',
    eachMemberPays: 'ஒவ்வொருவர் செலுத்துவது',
    monthlyCollectionSummary: 'மாதாந்திர வசூல் சுருக்கம்',
    noEligibleTickets: 'தகுதியான டிக்கெட்டுகள் இல்லை (அனைவரும் வென்றனர்)',
    fixedAmount: 'நிலையான தொகை',
    percentage: 'சதவீதம்',

    // Payments Extended
    totalInflow: 'மொத்த வரவு',
    totalPayouts: 'மொத்த செலவு',
    commissionEarned: 'கமிஷன் வருவாய்',
    netBalance: 'நிகர இருப்பு',
    totalInflowDesc: 'உறுப்பினர்களிடமிருந்து சேகரிக்கப்பட்ட பணம்',
    totalPayoutsDesc: 'வெற்றி தொகைகள் வழங்கப்பட்டது',
    ofInflow: 'வரவில்',
    surplus: 'உபரி',
    deficit: 'பற்றாக்குறை',
    groupBreakdown: 'குழு விவரம்',
    recentPayments: 'சமீபத்திய கட்டணங்கள்',
    member: 'உறுப்பினர்',
    totalPaid: 'மொத்த செலுத்தியது',
    totalDue: 'மொத்த செலுத்த வேண்டியது',
    remaining: 'மீதம்',
    payingMonths: 'செலுத்தும் மாதங்கள்',
    payment: 'கட்டணம்',
    allPayments: 'அனைத்து கட்டணங்கள்',
    paymentHistory: 'கட்டண வரலாறு',
    more: 'மேலும்',

    // Profile Extended
    activeAccount: 'செயலில் கணக்கு',
    personalInformation: 'தனிப்பட்ட தகவல்',
    updatePassword: 'கடவுச்சொல் புதுப்பி',

    noGroupSelected: 'குழு தேர்வு செய்யப்படவில்ளை',

    // Toast / messages
    groupCreated: 'சீட்டு குழு உருவாக்கப்பட்டது!',
    groupUpdated: 'குழு புதுப்பிக்கப்பட்டது!',
    groupCancelled: 'சீட்டு குழு ரத்து செய்யப்பட்டது',
    memberAdded: 'உறுப்பினர் சேர்க்கப்பட்டார்!',
    memberUpdated: 'உறுப்பினர் புதுப்பிக்கப்பட்டார்!',
    memberDeactivated: 'உறுப்பினர் செயலிழக்கப்பட்டார்',
    ticketAdded: 'டிக்கெட் சேர்க்கப்பட்டது!',
    paymentRecorded: 'கட்டணம் பதிவு செய்யப்பட்டது!',
    passwordUpdated: 'கடவுச்சொல் புதுப்பிக்கப்பட்டது',
    failedToLoad: 'ஏற்றுவதில் தோல்வி',

    // Reports
    downloadReport: 'அறிக்கை பதிவிறக்கம்',
    downloadPdf: 'PDF பதிவிறக்கம்',
    selectLanguage: 'மொழி தேர்வு',
    english: 'ஆங்கிலம்',
    tamil: 'தமிழ்',
    allGroupsCombinedReport: 'அனைத்து குழுக்களும் ஒருங்கிணைக்கப்பட்டது',
    eachGroupSeparate: 'ஒவ்வொரு குழுவும் தனித்தனி',
    selectReportType: 'அறிக்கை வகையை தேர்ந்தெடுக்கவும்',
    generating: 'உருவாக்கப்படுகிறது...',
    groupReport: 'குழு அறிக்கை',
    auctionReport: 'ஏல அறிக்கை',
    memberReport: 'உறுப்பினர் அறிக்கை',
    chitPool: 'சீட்டு தொகுப்பு',
    bidDiscount: 'வெற்றியாளரின் தியாகம்',
    bidAmount: 'ஏல தொகை',
    result: 'முடிவு',
    totalPool: 'ஈவுத்தொகை குளம்',
    rawPerMember: 'ஒவ்வொருவருக்கும் மூல தொகை',
    roundoffBalance: 'சுற்று இருப்பு',
    fixed: 'நிலையான',
    dividendPool: 'ஈவுத்தொகை கணக்கீடு',
    monthlyCollection: 'மாதாந்திர வசூல்',
    carryFromPrevious: 'முந்தைய இருப்பு',
    carryToNext: 'அடுத்ததற்கு கொண்டு செல்',
    monthlyContribution: 'மாதாந்திர பங்களிப்பு',
    dividendPerMember: 'ஒவ்வொருவருக்கும் ஈவுத்தொகை',
    roundedDividend: 'சுற்றப்பட்ட ஈவுத்தொகை',
  },
} as const;

export type Language = keyof typeof translations;
export type TranslationKey = keyof typeof translations.en;