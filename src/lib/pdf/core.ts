...
import { showPdfSaveDialog } from '@/components/ui/PdfSaveDialog';
...
export function drawCalcLedger(
  ...
  rows.forEach((row) => {
    ...
    // Value — right-aligned, formatted with symbol
    ...
    // Label text — starts from marginX
    doc.setFontSize(isResult ? 8.5 : 8);
    doc.setFont('helvetica', isResult ? 'bold' : 'normal');
  ...
  // Section-end rule — drawn below the last row with a clear gap
  y += 3;
  doc.setDrawColor(...COLORS.border);
  doc.setLineWidth(0.3);
...
// --- Save / Share dialog -----------------------------------------------------
// Shows a React-based popup with Download and Share options (uses React Icons).
export async function savePdf(doc: jsPDF, filename: string): Promise<void> {
  return showPdfSaveDialog(doc, filename);
}
